...
{
	vector<double> testVector = {2, 3, 4};
	DoubleOddDecreaseEven(testVector);
	BOOST_CHECK_EQUAL(testVector[0], 4);
	BOOST_CHECK_EQUAL(testVector[1], -6);
	BOOST_CHECK_EQUAL(testVector[2], 8);
}

BOOST_AUTO_TEST_CASE(convert_positive_vector)
{
	vector<double> testVector = { 2, 3, 4 };
	ConvertVector(testVector);
	BOOST_CHECK_EQUAL(testVector[0], -6);
	BOOST_CHECK_EQUAL(testVector[1], 4);
	BOOST_CHECK_EQUAL(testVector[2], 8);
}
...
BOOST_AUTO_TEST_CASE(convert_simple_vector_without_sort)
{
	...
	DoubleOddDecreaseEven(testVector);
	BOOST_CHECK_EQUAL(testVector[0], -10);
	BOOST_CHECK_EQUAL(testVector[1], -8);
	BOOST_CHECK_EQUAL(testVector[2], -2);
	BOOST_CHECK_EQUAL(testVector[3], 4);
}
